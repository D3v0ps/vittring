...
class TendSignScraper(BaseScraper[ProcurementItem]):
    name: ClassVar[str] = "tendsign"
    base_url: ClassVar[str] = "https://www.opic.com"
    domain: ClassVar[str] = "www.opic.com"
    source_value: ClassVar[str] = "tendsign"
    ...
    def parse(self, body: str, url: str) -> ProcurementItem:
        ...
        raise NotImplementedError("TendSignScraper.parse is not yet implemented")
    ...
    async def fetch_since(self, since: datetime) -> AsyncIterator[ProcurementItem]:
        ...
        for url in urls:
            ...
            try:
                ...
                logger.warning("tendsign_parse_failed", url=url, error=str(exc))
                continue
            yield item
